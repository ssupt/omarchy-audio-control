...
Panel {
  ...
  readonly property string panelError: !audioService || !audioService.ready
    ? "Audio service is unavailable" : audioService.error !== "" ? audioService.error : defaultDeviceError !== ""
  ...
  function setAudioNodeVolume(node, value, maximum) {
    ...
    try {
      return !!audioService && audioService.changeNode(node, { volume: Math.max(0, Math.min(ceiling, requested)) })
    } catch (_error) {
      return false
    }
  }
  ...
  function setAudioNodeMuted(node, muted) {
    if (!mutableAudioNode(node)) return false
    try {
      return !!audioService && audioService.changeNode(node, { muted: muted === true })
    } catch (_error) {
      return false
    }
  }
  ...
  function toggleAudioNodeMute(node) {
    if (!mutableAudioNode(node)) return false
    try {
      return !!audioService && audioService.changeNode(node, { muted: node.audio.muted !== true })
    } catch (_error) {
      return false
    }
  }
  ...
  function adjustVolume(delta) {
    ...
    if (focusSection === "streams" && selectedIndex >= 0 && selectedIndex < displayAudioStreams.length) {
      ...
      if (isFinite(playbackVolume)) setAudioNodeVolume(s, playbackVolume, outputVolumeMaximum)
      return
    }
  ...
  function enforceOutputVolumeLimit() {
    if (outputOverdrive || directDeviceMutationBusy) return
    if (outputVolume > 1) setOutputVolume(1)
    for (var i = 0; i < candidateStreams.length; i++) {
      var stream = candidateStreams[i]
      if (audioNodeVolume(stream) > 1) setAudioNodeVolume(stream, 1, 1)
    }
  }
            ...
            Repeater {
              ...
              AudioStreamRow {
                ...
                onVolumeRequested: function(value) {
                  root.setAudioNodeVolume(streamDelegate.node, value, streamDelegate.volumeMaximum)
                }
          ...
          Column {
            ...
            Repeater {
              ...
              AudioStreamRow {
                ...
                onVolumeRequested: function(value) {
                  root.setAudioNodeVolume(recordingStreamDelegate.node, value, recordingStreamDelegate.volumeMaximum)
                }
