...
Item {
  ...
  function write(args) {
    ...
    var params = ({})
    if (args[0] === "set-app") {
      method = "rules.set_app"; params = { app: args[1], direction: args[2], target: args[3] }
    } else if (args[0] === "del-app") {
      method = "rules.delete_app"; params = { app: args[1], direction: args[2] }
    } else if (args[0] === "set-alias") {
      method = "devices.alias"; params = { node: args[1], label: args[2] }
    } else if (args[0] === "set-flag") {
      method = "devices.flag"; params = { node: args[1], flag: args[2], value: args[3] === "true" }
    } else return false
    error = ""
    writing = true
    service.request(method, params, function(_result, failure) {
      root.writing = false
      root.error = failure ? failure.message : ""
      root.writeFinished(!failure)
    })
    return true
  }
